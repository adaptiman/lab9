...
[
    {
        "Name": "bridge",
        "Id": "947f73719557ea6a5cd404586d7c1598a73a5e06d114b0bb4f4b51621a48ffb1",
        "Created": "2020-04-08T02:36:07.0416993Z",
        "Scope": "local",
        "Driver": "bridge",
        "EnableIPv6": false,
        "IPAM": {
        ...
        "Containers": {
            "224a97d6bc54829f8be84b48d230f06ff316d07da150c1a4df4e74baeceeb5a3": {
                "Name": "funny_wilson",
                "EndpointID": "97ff5f9cd227b40e75c7e4349d9619f7bef064df0af15c75ea9e18d810520523",
...
You can see that our container `224a97d6bc` is listed under the `Containers` section in the output. What we also see is the IP address this container has been allotted - `172.17.0.2`. Is this the IP address that we're looking for? Let's find out by running our flask container and trying to access this IP.
...
[
    {
        "Name": "foodtrucks",
        "Id": "2d2d43bbeb82c154f65f1bcc4161905b03bb804c58359bc6b030198b169edd0f",
        "Created": "2020-04-08T05:44:55.449617716Z",
        "Scope": "local",
        "Driver": "bridge",
        "EnableIPv6": false,
        "IPAM": {
        ...
        "Containers": {
            "6479218d11a99208ae827ea4950c90142c39f60a88f4dfc272cb17f487f86279": {
                "Name": "es",
                "EndpointID": "fcb708426fbd68978ac71f7462f46c5e9340f85274b46e76c5266f2165dbb05e",
...
```javascript
$ docker run -it --rm --net foodtrucks adaptiman/foodtrucks-web bash
root@e51409dad298:/opt/flask-app# cat /etc/hosts
127.0.0.1       localhost
...
172.18.0.3      e51409dad298
root@e51409dad298:/opt/flask-app# curl es:9200
{
  "name" : "6479218d11a9",
  "cluster_name" : "docker-cluster",
...
root@e51409dad298:/opt/flask-app# ls
app.py  node_modules  package-lock.json  package.json  requirements.txt  static  templates  webpack.config.js
root@e51409dad298:/opt/flask-app# python app.py
Index not found...
Loading data in elasticsearch ...
Total trucks loaded:  653
 * Running on http://0.0.0.0:5000/ (Press CTRL+C to quit)
root@e51409dad298:/opt/flask-app# exit
```
...
```javascript
$ docker run -d --net foodtrucks -p 5000:5000 --name foodtrucks-web adaptiman/foodtrucks-web
7b961568b270523fbe9e5cd21b825df88fdf5e86706717741d30abaaa3ff876e
...
Content-Length: 3697
Server: Werkzeug/0.11.2 Python/2.7.17
Date: Wed, 08 Apr 2020 06:15:05 GMT
```

Head over to http://\<yourdockerdevmachineIP\>:5000 and see your glorious app live! Although that might have seemed like a lot of work, we actually just typed 4 commands to go from zero to running. I've collated the commands in a [bash script](https://raw.githubusercontent.com/adaptiman/lab9/master/FoodTrucks/setup-docker.sh).
```
#!/bin/bash
...
```javascript
$ git clone https://github.com/adaptiman/Lab9
$ cd Lab9/FoodTrucks
$ ./setup-docker.sh
```
